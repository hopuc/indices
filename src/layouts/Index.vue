<template>
	<!-- <div id="app"> -->
		...
		<!-- <q-container> -->
			...
			<!-- <q-header> -->
				<!-- <q-tabs v-model="index" @tab-click="handleClick">
					<q-tab-pane label="用户管理" name="table">用户管理</q-tab-pane>
					<q-tab-pane label="配置管理" name="chart">配置管理</q-tab-pane>
					<q-tab-pane label="角色管理" name="third">角色管理</q-tab-pane>
				</q-tabs> -->
				<!-- <q-menu :default-active="activeIndex" mode="horizontal" @select="handleSelect" class="">
					<q-menu-item index="1" @click="drawer = true" type="primary"><i class="q-icon-menu"></i></q-menu-item>
					<q-menu-item index="table">表格</q-menu-item>
					<q-menu-item index="chart">图表</q-menu-item>
				</q-menu> -->
			<!-- </q-header> -->
			...
			<!-- <q-drawer
				...
				<q-menu
				default-active="1"
				class="q-menu-vertical-demo"
				@select="drawerSelect"
				>
					<q-submenu index="1" :show-timeout="100">
						<template slot="title">设置</template>
						<q-menu-item index="alias">
							<q-row>
								<q-col :span="18">简称模式</q-col>
								<q-col :span="6"><q-switch v-model="alias" active-color="#1a6" :width="36"></q-switch></q-col>
							</q-row>
						</q-menu-item>
						<q-menu-item index="panel">
							<q-row>
								<q-col :span="18">操作面板</q-col>
								<q-col :span="6"><q-switch v-model="panel" active-color="#1a6" :width="36"></q-switch></q-col>
							</q-row>
						</q-menu-item>
						<q-menu-item index="fixed">
							<q-row>
								<q-col :span="18">固定日期</q-col>
								<q-col :span="6"><q-switch v-model="fixed" active-color="#1a6" :width="36"></q-switch></q-col>
							</q-row>
						</q-menu-item>

					</q-submenu>
					<q-submenu index="2" :show-timeout="200">
						<template slot="title">指标数据</template>
						<q-menu-item index="202001">1月</q-menu-item>
						<q-menu-item index="202002">2月</q-menu-item>
						<q-menu-item index="202003">3月</q-menu-item>
						<q-menu-item index="202004">4月</q-menu-item>
						<q-menu-item index="202005">5月</q-menu-item>
						<q-menu-item index="202006">6月</q-menu-item>
						<q-menu-item index="2020">2020年</q-menu-item>
					</q-submenu>
					<q-submenu index="3" :show-timeout="100">
						<template slot="title">添加数据</template>
						<q-menu-item index="add" @click="openAdd()">窗口添加</q-menu-item>
						<q-menu-item index="addRow" @click="addRow()">表格添加</q-menu-item>
					</q-submenu>
					<q-submenu index="4" :show-timeout="100">
						<template slot="title">下载指标</template>
						<q-menu-item index="download" @click="tocsv(1,'车间指标统计表.csv')">车间指标统计表</q-menu-item>
						<q-menu-item index="dl" @click="tocsv(2,'指标统计表.csv')">指标统计表</q-menu-item>
					</q-submenu>
					<q-menu-item index="copy" @click="openCopy()">复制指标</q-menu-item>
				</q-menu>
			</q-drawer> -->
			...
			<q-row>
				<q-input id="add" type="textarea" :rows="5" v-model="addText" placeholder="请输入内容"></q-input>
			</q-row>

			<span slot="footer" class="dialog-footer">
				<q-button type="primary" @click="add()">添加</q-button>
				<q-button @click="addDialog = false">取消</q-button>
			</span>
			</q-dialog>
			...
			width="500px"
			>
			<q-input id="copy" type="textarea" :rows="5" v-model="totalText" placeholder="暂无内容"></q-input>
			<span slot="footer" class="dialog-footer">
				<q-button type="primary" @click="copy()">复制</q-button>
				<q-button @click="copyDialog = false">取消</q-button>
			</span>
			</q-dialog> -->


			<!-- <q-main> -->
				<!-- <div v-show="index=='table'">
					<q-table
					v-loading="loading"
					element-loading-text="加载中"
					element-loading-spinner="q-icon-loading"
					element-loading-background="rgba(225, 225, 225, 0.7)"
					:data="json"
					height="99.8vh"
					class="table"
					ref="table"
					...
					>
						...
						:key="k"
						:prop="k"
						:label="alias?v.alias:v.name"
						:width="v.width"
						:fixed="fixed&&(k=='date'||k=='shift')"
						sortable
						>
						<template slot-scope="scope"> -->

							<!-- <span v-if="scope.$index==edit&&v.writable">
								<q-date-picker
								v-if="k=='date'"
								size="small"
								...
								<span v-else>
									<q-popover
									placement="top-start"
									:title="scope.row['date']+' '+scope.row['shift']"
									width="200"
									trigger="hover"
									:content="v.name">
										<q-input slot="reference" v-popover:popover size="small" class="input" :placeholder="v.name" v-model="scope.row[k]" clearable>
										</q-input>
									</q-popover>
								</span>
							</span> -->
							<!-- <span v-else>
								{{scope.row[k] | rounding(v.decimal)}}
							</span> -->
						<!-- </template>
						</q-table-column> -->

						<!-- <q-table-column
						v-if="panel&&permission>0"
						fixed="right"
						label="操作"
						width="90">
							<template slot-scope="scope">
								...
								@click.native.prevent="editRow(scope.$index, json);calculateData(scope.$index);save()"
								type="text"
								size="small"
								>
								保存
								...
								<q-button
								v-if="edit==scope.$index"
								@click.native.prevent="editRow(scope.$index, json)"
								type="text"
								size="small"
								>
								取消
								...
								<q-button
								v-if="permission>1&&edit!==scope.$index"
								@click.native.prevent="message(scope.$index, json)"
								type="text"
								size="small"
								>
								删除
				...
				<!-- <div class="panel">
					<q-button
					@click.native.prevent="addRow();notice('添加成功','请滚动至表格最下方输入相应的数据！')"
					type="primary"
					size="small"
					>
					添加
					...
					<q-button
					@click.native.prevent="switchPanel()"
					type="warn"
					size="small"
					>
					编辑
    ...
    <q-header elevated class="bg-primary">
      <q-toolbar>
        <q-btn show-if-above flat @click="drawer = !drawer" round dense icon="menu" />
    ...
    <q-table
      ...
      table-style="max-height: calc(100vh - 164px)"
      virtual-scroll
        ...
        if(index>0){
          ...
          id:'total',
			...
			openCopy: function(){
				...
				var date = ''
				if(this.selectedData.length == 2 && this.selectedData[0].date == this.selectedData[1].date){
					date = this.selectedData[0].date
				}
				var cuc_deh_cu = this.selectedData.reduce((total, v) => {
					console.log(total , v.cuc_deh_cu)
					return total + Number(v.cuc_deh_cu/this.selectedData.length)
				}, 0)
				var fec_deh_fe = this.selectedData.reduce((total, v) => {
					console.log(total , v.fec_deh_fe)
					return total + Number(v.fec_deh_fe/this.selectedData.length)
				}, 0)
...
/* 	.q-table td, .q-table th{
	...
	.q-date-editor.q-input, .q-date-editor.q-input__inner {
		width: 80px !important;
	}
	.q-row {
		margin-bottom: 10px;
	} */
	/* .q-date-editor .q-input__inner {
